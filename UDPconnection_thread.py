...
class commBuffer():
    ...
    def confirm(self, seqNum, time=None):
        ...
        if time is not None and msg and msg[TIME]:
            ping = time - msg[TIME]
            self.addping(ping, time)

    def addping(self, ping, time=0):
        if time:
            self.lastPingAdded = time
...
#        taskMgr.doMethodLater(0.5, self.connect, "Connect", extraArgs=[userdata], appendTask=True)
        self.connectThread = threading.Thread(target=self.connect, args=(userdata,))
